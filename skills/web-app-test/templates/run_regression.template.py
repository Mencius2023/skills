...
按测试金字塔 0->4 层顺序执行，任一层失败立即中断并返回该退出码（fail-fast）：
  0  冒烟 + 数据完整性
  1  前端单元测试（存在才跑）
  2  配置回归 + 离线核心功能
  3  前端生产构建
  4  浏览器 E2E（Playwright，连真实后端）

第 4 层跑 spec 中两部分：预先设计的 API 契约断言（覆盖主要端点）+ 已沉淀的稳定浏览器流程。
脆弱、易变的浏览器 UI 交互不在本运行器内——由 AI 动态接入浏览器执行（参考 L4_OPERATION_GUIDE.md），
测完后判断需回归才沉淀为 spec。
...
def main() -> int:
    ...
    parser.add_argument("--quick", action="store_true", help="只跑 0/2 层快检")
    parser.add_argument("--reuse-ui", action="store_true", help="复用已运行的前端做 E2E")
    args = parser.parse_args()

    config = SKILL_DIR / "scripts" / "playwright.config.js"

    # 第 0 层：冒烟 + 数据完整性
    run("L0 冒烟测试", [sys.executable, str(BACKEND_TESTS / "<smoke_test.py>")], PROJECT_ROOT)
    # 示例：非阻塞审计步骤（如有），用 allow_fail=True
    # run("L0 数据完整性审计", ["node", str(EDITOR_ROOT / "scripts" / "<audit>.mjs")], EDITOR_ROOT, allow_fail=True)

    # 第 1 层：前端单元测试（存在才跑）
    if has_unit_test_script():
        run("L1 前端单元测试", [npm(), "run", "test:unit"], EDITOR_ROOT)
    else:
        print("[SKIP] L1 前端单元测试：package.json 暂无 test:unit 脚本")

    # 第 2 层：配置回归 + 离线核心功能
    run("L2 配置回归", [sys.executable, str(BACKEND_TESTS / "<config_regression.py>")], PROJECT_ROOT)
    run("L2 离线标准流程", [sys.executable, str(BACKEND_TESTS / "<standard_flow_test.py>")], PROJECT_ROOT)

    if args.quick:
        print("=" * 80)
        print("[PASS] 快检通过（L0 + L2 核心）")
        return 0

    run("L2 核心功能测试", [sys.executable, str(BACKEND_TESTS / "<test_core_functions.py>")], PROJECT_ROOT)

    # 第 3 层：前端生产构建
    run("L3 前端生产构建", [npm(), "run", "build"], EDITOR_ROOT)

    # 第 4 层：浏览器 E2E（Playwright）— API 契约（预设计）+ 已沉淀的稳定浏览器流程
    if not args.skip_browser:
        env_note = "(复用已运行前端)" if args.reuse_ui else "(自动拉起前端)"
        cmd = [npx(), "playwright", "test", "--config", str(config)]
        run(f"L4 浏览器 E2E(Playwright){env_note}", cmd, EDITOR_ROOT)
    else:
        print("[SKIP] L4 浏览器 E2E（--skip-browser）")
