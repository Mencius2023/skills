...
|---------|--------|------|
| `TEST_SPECIFICATION.template.md` | `web-app-test/TEST_SPECIFICATION.md` | 测试说明文档（主文档）骨架。开头声明所属产品，分层组织用例，附录登记选择器契约、fixture、环境配置、**脚本清单与链接** |
| `run_regression.template.py` | `web-app-test/scripts/run_regression.py` | 非交互回归运行器（CI 入口），按测试金字塔分层 fail-fast。**预先设计**，落地即填好 |
...
3. **脚本设计策略分两类**：
   - **程序测试脚本（后端/API/构建，L0–L3、L5）必须预先设计**，并覆盖产品主要功能（核心数据、关键配置、核心流程、主要 API、错误处理、生产构建等）。`run_regression.py` 与各后端测试脚本属于此类，落地时就要写好。
   - **浏览器 E2E 的 Playwright spec（L4）不预先设计**，由 AI 动态接入浏览器测完功能后，现场判断是否需回归再沉淀。`standard-flow.spec.template.js` 只是一个**沉淀时**可参考的骨架，不要求落地即写满所有 L4 用例。
4. **规格书不是设计文档**：每条用例必须挂有实际可执行资源（脚本 / 可直接运行的命令）或明确标为 AI 动态执行。
